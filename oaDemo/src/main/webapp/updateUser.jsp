	...
	$(function() {
		$("#saveBtn").click(
				function() {
					...
					// 使用ajax进行异步数据提交
					if (res) {
						$.post("savaUpdateUser.do", $("#form").serialize(),
								function(data) {
									alert(data.msg);
									//关掉当前弹出窗口,刷新父界面
	...
	<form id="form" method="post">
		<table>
			<tr>
				...
				<td class="text-content"><input type="hidden" name="userId"
					value="${user.userId}"><input type="text" name="userName"
					value="${user.userName }"
			...
			<tr>
				...
				<td class="text-content"><input type="password" name="password"
					value="${user.password}"
					class="easyui-textbox theme-textbox-radius"
					data-options="required:true,validType:'length[6,20]'"></td>
			</tr>
			...
			<tr>
				...
				<td class="text-content"><input type="text" name="phone"
					value="${user.phone}" class="easyui-textbox theme-textbox-radius"
					data-options="required:true"></td>
			</tr>
			<tr>
				<td class="text-title">电子邮箱：</td>
				<td class="text-content"><input type="text" name="email"
					value="${user.email }" class="easyui-textbox theme-textbox-radius"
